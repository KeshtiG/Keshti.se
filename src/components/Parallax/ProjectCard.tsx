...
export function ProjectCard({ project }: { project: Project }) {
  ...
  return (
    <section className={`${styles["parallax-container"]} h-svh w-full px-0 md:px-8 lg:px-16 xl:px-24 flex flex-col justify-center items-center lg:flex-row gap-0 md:gap-4 lg:gap-8 xl:gap-16 `}>
      <div ref={ref} className="w-full h-[40vh] lg:w-3/5 lg:h-[60vh] relative overflow-hidden md:rounded-lg shrink-0">
        <img src={project.imgUrl} alt={project.title} className="absolute inset-0 w-full h-full object-cover object-center" />
      </div>
      <motion.div
        initial={{ visibility: "hidden" }}
        animate={{ visibility: "visible" }}
        style={{ y }}
        className="w-11/12 lg:w-2/5 flex flex-col justify-center bg-neutral-800/60 backdrop-blur-sm lg:bg-background border border-neutral-700 lg:border-background p-4 md:p-8 lg:p-0 rounded-lg gap-4 md:gap-6 will-change-transform"
      >
